...
def combine_data(input_path):
	...
	for file in tqdm(file_list):
		...
		data = pd.read_csv(filename, converters = {"post_id": str, "post_name": str, "post_message": str, "post_description": str },  lineterminator = '\n')
...
def select_posts(data, output_path):
	...
	for i in tqdm(range(0, len(post_data["post_id"]))):
		...
		elif post.find("muslim") != -1 or post.find("muslims") != -1:
			if post.find("trump") != -1:
				immigration_index.extend([i])
			elif post.find("ban") != -1:
				immigration_index.extend([i])

	immigration_posts = post_data.iloc[immigration_index]
	immigration_posts.index.name = "index"
	immigration_posts.to_csv(output_path + "immigration_posts_event2.csv")
	...
	none_posts.to_csv(output_path + "immigration_none_posts_event2.csv")


def main():
	data = "/home3/r05322021/Desktop/FB Data/posts_rawdata/immigration_event2/immigration_event2.csv"
